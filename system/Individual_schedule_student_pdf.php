...
function generateAndDownloadPDF($conn, $student_code) {
    ...
    $sql = "SELECT * FROM merged_data WHERE student_code = :student_code GROUP BY exam_day, exam_date, exam_time, timeslot_subject_code order by exam_date";
    ...
    if (count($data) > 0) {
        ...
        $pdf->Cell(0, 5, 'Exam Schedule for student: ' . $student_code, 0, 1, 'C');
...
if (isset($_POST['download_pdf']) && isset($_POST['student_code'])) {
    $student_code = $_POST['student_code'];
    generateAndDownloadPDF($conn, $student_code);
} else {

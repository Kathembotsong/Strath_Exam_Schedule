...
function generateAndDownloadPDF($conn, $lecturer_name) {
    ...
    $sql = "SELECT * FROM merged_data WHERE timeslot_lect_name = :lecturer_name GROUP BY exam_day, exam_date, exam_time, timeslot_subject_code order by exam_date";
    ...
    if (count($data) > 0) {
        ...
        $pdf->Cell(0, 5, 'Exam Schedule for Lecturer: ' . $lecturer_name, 0, 1, 'C');
        ...
        foreach ($data as $row) {
            ...
            $pdf->Cell(40, 10, $row['timeslot_subject_name'], 1, 0, 'C', 0, '', 1);
...
if (isset($_POST['download_pdf']) && isset($_POST['lecturer_name'])) {
    $lecturer_name = $_POST['lecturer_name'];
    generateAndDownloadPDF($conn, $lecturer_name);
} else {

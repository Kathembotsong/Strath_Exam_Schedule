...
$subjectQuery = "SELECT subject_code, subject_name, group_name, lect_name FROM enrollments_tourism";
...
function getGroupCapacity($conn, $group) {
    $sql = "SELECT group_capacity FROM group_tourism WHERE group_name = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$group]);
